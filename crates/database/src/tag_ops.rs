...
impl<'a> TagOp<'a> {
    ...
    pub async fn add(
        ...
        let hide = if permissions::can_set_hide(actor_level) {
            hide_username
        } else {
            false
        };
    ...
    pub async fn modify(
        ...
        self.repo
            .update_tag(uuid, tag_type, new_reason, hide)
            .await?
    ...
    pub async fn overwrite(
        ...
        let old_tag = self
            .repo
            .get_tag_by_id(old_tag_id)
            .await?
            .ok_or(TagOpError::TagNotFound)?;

        let is_own = old_tag.added_by == actor_id;
        let age = Utc::now()
            .signed_duration_since(old_tag.added_on)
            .num_minutes();
        ...
        self.repo
            .add_tag(uuid, new_type, reason, actor_id, hide, None)
            .await?;

        let new_tag = self
            .repo
            .get_tag_by_type(uuid, new_type)
            .await?
            .ok_or(TagOpError::Database(sqlx::Error::RowNotFound))?;
    ...
    pub async fn unlock_player(&self, uuid: &str, actor_level: i16) -> Result<bool, TagOpError> {
        if actor_level < 3 {
            return Err(TagOpError::ModeratorRequired);
        }
    ...
    async fn find_priority_conflict(
        &self,
        uuid: &str,
        tag_type: &str,
    ) -> Result<Option<PlayerTagRow>, TagOpError> {
