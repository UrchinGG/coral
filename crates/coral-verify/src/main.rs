...
use coral_verify::api::CoralApiClient;
use coral_verify::framework::{Data, Handler};
...
async fn main() -> Result<()> {
    ...
    tracing::info!("Starting Coral Verify");
    client.start().await?;
    Ok(())
}
...
fn init_logging() {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("info,serenity=warn")
    });
...
async fn build_client(data: Data) -> Result<Client> {
    let token = Token::from_env("CORAL_VERIFY_DISCORD_TOKEN").expect("Invalid CORAL_VERIFY_DISCORD_TOKEN");
