...
pub async fn handle_link_pick(
    ...
    match crate::accounts::check_link(data, uuid, &discord_name).await {
        crate::accounts::LinkCheck::Verified { uuid, username } => {
            ...
            let _ = CacheRepository::new(data.db.pool()).cache_username(&uuid, &username).await;
...
pub async fn handle_add_code_modal(
    ...
    crate::accounts::link_alt(ctx, data, target_id, member.id, &uuid).await?;
    let _ = CacheRepository::new(data.db.pool()).cache_username(&uuid, &player.username).await;
...
pub async fn handle_add_account_modal(
    ...
    if verified {
        crate::accounts::link_alt(ctx, data, target_id, member.id, &uuid).await?;
        let _ = CacheRepository::new(data.db.pool()).cache_username(&uuid, &stats.username).await;
