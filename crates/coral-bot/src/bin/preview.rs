...
fn cards() -> Vec<Card> {
    vec![
        ...
        Card {
            ...
            needs_player: false,
            render: |_, _| {
                coral_bot::commands::stats::bedwars::cards::prestiges::render_prestiges()
            },
        },
    ]
}
...
fn main() {
    ...
    let (player_data, extra_args) = if card.needs_player {
        ...
        let http = Client::builder()
            ...
            .unwrap();
        (
            fetch_player_data(&http, &api_key, player_name),
...
fn fetch_player_data(http: &Client, api_key: &str, name: &str) -> PlayerData {
    ...
    let guild: Value = http
        .get(format!("https://api.hypixel.net/v2/guild?player={uuid}"))
        .header("API-Key", api_key)
        .send()
        .unwrap()
        .json()
...
fn fetch_skin(http: &Client, uuid: &str) -> Option<DynamicImage> {
    ...
    let decoded =
        base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &prop.value).ok()?;
