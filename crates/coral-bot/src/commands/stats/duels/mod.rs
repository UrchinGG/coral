...
use hypixel::{
    DuelsStats, DuelsView, DuelsWinstreak, DuelsWinstreakSnapshot, GuildInfo, extract_duels_stats,
    extract_duels_winstreak_snapshot,
};
...
use super::{
    AutoPreset, GameStats, OverallCache, SessionCacheEntry, encode_png, extract_select_value,
    overall, session,
};
...
impl GameStats for Duels {
    ...
    fn render_session(
        ...
        let ws = duels_winstreaks::calculate(snapshots, *mode);
        encode_png(&render_duels_session(
            current,
            previous,
            session_type,
            started,
    ...
    fn format_delta(current: &DuelsStats, previous: &DuelsStats, _mode: &DuelsView) -> String {
        ...
        let kills = current
            .overview
            .kills
            .saturating_sub(previous.overview.kills);
        let deaths = current
            .overview
            .deaths
            .saturating_sub(previous.overview.deaths);
        let kd = if deaths == 0 {
            kills as f64
        } else {
...
async fn detect_auto_presets_duels(
    cache_repo: &CacheRepository<'_>,
    uuid: &str,
    ws_hidden: bool,
) -> Vec<AutoPreset> {
