...
use crate::{
    ...
    commands::{
        ...
        stats::{
            OverallCache, SessionCacheEntry,
            bedwars::Bedwars, duels::Duels,
        },
    },
};
...
impl Handler {
    ...
    async fn handle_component_direct(
        ...
        match id {
            ...
            "session_duels_mode" => commands::stats::duels::handle_session_mode_switch(ctx, component, &self.data).await,
            "session_duels_switch" => commands::stats::duels::handle_session_switch(ctx, component, &self.data).await,
            "session_mode" => commands::stats::bedwars::handle_session_mode_switch(ctx, component, &self.data).await,
            "session_switch" => commands::stats::bedwars::handle_session_switch(ctx, component, &self.data).await,
            _ if id.starts_with("session_mgmt_rename:") => commands::stats::bedwars::handle_mgmt_rename_button(ctx, component, &self.data).await,
            _ if id.starts_with("session_mgmt_delete:") => commands::stats::bedwars::handle_mgmt_delete_button(ctx, component, &self.data).await,
            _ if id.starts_with("session_confirm_delete:") => commands::stats::bedwars::handle_confirm_delete_button(ctx, component, &self.data).await,
            _ if id.starts_with("session_duels_mgmt_rename:") => commands::stats::duels::handle_mgmt_rename_button(ctx, component, &self.data).await,
            _ if id.starts_with("session_duels_mgmt_delete:") => commands::stats::duels::handle_mgmt_delete_button(ctx, component, &self.data).await,
            _ if id.starts_with("session_duels_confirm_delete:") => commands::stats::duels::handle_confirm_delete_button(ctx, component, &self.data).await,
            ...
            _ if id.starts_with("mt_remove:") => commands::blacklist::tag::handle_manage_remove(ctx, component, &self.data).await,
            _ if id.starts_with("mt_confirm:") => commands::blacklist::tag::handle_manage_confirm(ctx, component, &self.data).await,
            _ if id.starts_with("mt_add:") => commands::blacklist::tag::handle_manage_add_select(ctx, component, &self.data).await,
            _ if id.starts_with("mt_back:") => commands::blacklist::tag::handle_manage_back(ctx, component, &self.data).await,
            _ if id.starts_with("mt_history:") => commands::blacklist::tag::handle_manage_history(ctx, component, &self.data).await,
    ...
    async fn handle_modal(&self, ctx: &Context, modal: &ModalInteraction) -> anyhow::Result<()> {
        ...
        match id {
            _ if id.starts_with("session_rename_modal:") => commands::stats::bedwars::handle_rename_modal(ctx, modal, &self.data).await,
            _ if id.starts_with("session_duels_rename_modal:") => commands::stats::duels::handle_rename_modal(ctx, modal, &self.data).await,
            ...
            _ if id.starts_with("mt_reason:") => commands::blacklist::tag::handle_manage_reason_modal(ctx, modal, &self.data).await,
            _ if id.starts_with("mt_expiry:") => commands::blacklist::tag::handle_manage_expiry_modal(ctx, modal, &self.data).await,
            _ if id.starts_with("manage_register_modal:") => commands::admin::manage::handle_register_modal(ctx, modal, &self.data).await,
            _ if id.starts_with("manage_dev_rate_limit_modal:") => commands::admin::manage::handle_dev_rate_limit_modal(ctx, modal, &self.data).await,
                ...
                if !event.user.bot() && event.user.id != ctx.cache.current_user().id {
                    ...
                    tokio::spawn(async move {
                        match guild_id.member(&ctx.http, user_id).await {
                            Ok(member) => crate::sync::handle_member_update(&ctx, &data, &member).await,
