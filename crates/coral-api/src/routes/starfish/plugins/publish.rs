...
pub async fn publish_plugin(
    ...
    let permission =
        github_api::get_permission(&req.github_access_token, &req.repo, &github_username).await?;
    if !matches!(permission.as_str(), "admin" | "maintain" | "write") {
        return Err(ApiError::Forbidden("insufficient_repo_permission".into()));
    }

    let release =
        github_api::get_release_by_tag(&req.github_access_token, &req.repo, &req.release_tag)
            .await?;
    let asset = release
        .assets
        .iter()
    ...
    let plugin = match (existing_by_slug, existing_by_repo_id) {
        (Some(by_slug), _) if by_slug.owner_user_id != caller.user.id => {
            return Err(ApiError::Forbidden(
                "plugin_slug_owned_by_another_user".into(),
            ));
        }

        (Some(by_slug), Some(by_repo)) if by_slug.id != by_repo.id => {
            return Err(ApiError::Conflict(
                "slug_and_repo_id_belong_to_different_plugins".into(),
            ));
        }
        ...
        (None, Some(by_repo)) => {
            return Err(ApiError::Conflict(format!(
                "this GitHub repo is already published as '{}'",
                by_repo.slug
            )));
        }
    ...
    if plugins_repo
        .get_release_by_version(plugin.id, &req.version)
        .await?
        .is_some()
    {
        return Err(ApiError::Conflict(format!(
    ...
    plugins_repo
        .clear_old_body_caches(plugin.id, release_row.id)
        .await?;
...
fn verify_github_digest(asset: &github_api::Asset, computed: &[u8; 32]) -> Result<(), ApiError> {
    ...
    let claimed_hex = digest
        .strip_prefix("sha256:")
        .ok_or_else(|| ApiError::BadRequest(format!("unsupported digest format: {digest}")))?;
    let claimed =
        hex::decode(claimed_hex).map_err(|_| ApiError::BadRequest("invalid digest hex".into()))?;
