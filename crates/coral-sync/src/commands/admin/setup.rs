...
pub async fn run(ctx: &Context, command: &CommandInteraction, data: &Data) -> Result<()> {
    ...
    let (config, rules, member) =
        tokio::try_join!(repo.upsert(gid, uid), repo.get_role_rules(gid), async {
            member_repo.get_by_discord_id(uid).await
        },)?;

    let preview_ctx =
        build_preview_from_member(ctx, data, guild_id.get(), command.user.id.get(), member).await;
...
pub async fn handle_cancel_button(
    ...
    let guild_id =
        interact::parse_id(&component.data.custom_id).ok_or_else(|| anyhow!("invalid guild ID"))?;
...
pub async fn handle_nickname_clear_button(
    ...
    let guild_id =
        interact::parse_id(&component.data.custom_id).ok_or_else(|| anyhow!("invalid guild ID"))?;
    GuildConfigRepository::new(data.db.pool())
        .set_nickname_template(guild_id as i64, None)
        .await?;
    interact::update_message(
        ctx,
        component,
        vec![build_nickname_panel(guild_id, None, None)],
    )
    .await?;
...
pub async fn handle_nickname_modal(
    ...
    let guild_id =
        interact::parse_id(&modal.data.custom_id).ok_or_else(|| anyhow!("invalid guild ID"))?;
    ...
    GuildConfigRepository::new(data.db.pool())
        .set_nickname_template(guild_id as i64, template)
        .await?;
...
pub async fn handle_add_rule_modal(
    ...
    if repo
        ...
        .await?
        .iter()
        .any(|r| r.role_id == role_id as i64)
    {
        return interact::send_modal_error(
            ctx,
            modal,
...
pub async fn handle_nickname_reset_button(
    ...
    spawn_bulk_update(
        ...
        move |cancel| {
            Box::pin(async move {
                crate::sync::clear_nicknames(ctx_clone, data_clone, GuildId::new(guild_id), cancel)
                    .await
            })
        },
    );
...
pub async fn handle_role_strip_button(
    ...
    spawn_bulk_update(
        ...
        data,
        guild_id,
        &format!("Stripping <@&{role_id}>"),
        move |cancel| {
            Box::pin(async move {
                crate::sync::clear_role(
                    ctx_clone,
                    data_clone,
                    GuildId::new(guild_id),
                    RoleId::new(role_id),
...
async fn refresh_autorole(
    ...
    interact::update_message(
        ctx,
        component,
        build_autorole_components(&rules, guild_id, selected_role),
    )
    .await
}
...
async fn build_preview_context(
    ...
    let member = MemberRepository::new(data.db.pool())
        .get_by_discord_id(user_id as i64)
        .await
        .ok()??;
...
fn render_with_context(template: &str, preview_ctx: Option<&serde_json::Value>) -> Option<String> {
    let rendered =
        expr::render_template(template, preview_ctx?).to_truncated(crate::sync::NICKNAME_MAX_LEN);
...
async fn can_manage_role(
    ...
    let Some(target) = roles.get(&target_role) else {
        return false;
    };
...
pub fn spawn_bulk_update(
    ...
    heading: &str,
    task: impl FnOnce(
        crate::sync::CancelToken,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>>
    + Send
    + 'static,
) {
    let cancel = crate::sync::cancel_token();
    data.sync_cancel_tokens
        .lock()
        .unwrap()
        .insert(GuildId::new(guild_id), cancel.clone());
